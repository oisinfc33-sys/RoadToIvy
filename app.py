        ...
        for uni in seed["universities"]:
            cur.execute(
                ...
                INSERT INTO universities (
                    name, country, region, tier, ranking_band, acceptance_band, notes
                )
                VALUES (?, ?, ?, ?, ?, ?, ?)
                """,
                (
...
def score_program(student: Dict[str, Any], program: sqlite3.Row) -> Dict[str, Any]:
    ...
    readiness = round(
        (academic_score * 0.5) + (activities_score * 0.25) + (subject_score * 0.25),
        1,
    )
    ...
    if program["postgraduate_only"]:
        next_steps.append(
            "This target is postgraduate-only. You would first need a strong undergraduate degree before applying."
        )

    if avg < program["stretch_avg"]:
        next_steps.append(
            f"Lift academic average from {avg:.1f} toward {program['stretch_avg']:.0f}+ for realistic contention."
        )

    if missing_subjects:
        next_steps.append(
            "Add or prove strength in required subjects: " + ", ".join(missing_subjects) + "."
        )

    if weak_subjects:
        next_steps.append(
            "Upgrade weak core subjects: " + ", ".join(weak_subjects) + "."
        )

    if ec_score < program["stretch_ec_score"]:
        next_steps.append(
            "Build a more credible extracurricular profile with leadership, research, competition results, or impact."
        )

    if required_tests:
        next_steps.append(
            "Plan for required tests/components: " + ", ".join(required_tests) + "."
        )
    ...
    if not next_steps:
        next_steps.append(
            "Maintain grades, tighten narrative, and execute applications with precise positioning."
        )
...
def analyze():
    ...
    if not course:
        return render_template("index.html", error="Enter a target course first.")
    ...
    if not matches:
        return render_template(
            "index.html",
            error="No matching programs found. Try a broader course label such as computer science, economics, law, engineering, medicine, or design.",
        )
